...
import type { ImageNodeType, TextNodeType } from '@/types'
import { NodeLayout } from '../NodeLayout'

type ImageTransformProps = NodeProps<ImageNodeType> & { title: string }

/**
 * 图片转换模式 — AI 图片生成（与 tersa 1:1 对齐）
 *
 * 从上游文本/图片节点获取输入，调用 AI 生成图片
 */
export function ImageTransform({ data, id, type, title }: ImageTransformProps) {
    ...
    try {
      ...
      const res = await fetch(
        `${clientEnv.SUPER_PUBLIC_API_BASE_URL}/canvas/generate-image`,
        {
          ...
          credentials: 'include',
          body: JSON.stringify({ prompt, modelId: data.model }),
        }
      )
  ...
  return (
    <NodeLayout id={id} title={title} toolbar={toolbar} type={type}>
      {loading && (
        <div
          className="flex w-full animate-pulse items-center justify-center rounded-b-xl bg-[#242424]"
      ...
      {!(loading || data.generated?.url) && (
        <div
          className="flex w-full items-center justify-center rounded-b-xl bg-[#242424]"
          style={{ aspectRatio: '1/1' }}
        >
